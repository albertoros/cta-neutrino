...
def AddConstantValue(doc,value):
    ...
    addParameter(spatial, 'Value', 0, value, 1.0, 0.001, 100.0)
    return spatial
...
def AddConstantTrans(doc,value):
    temporal = doc.createElement('temporal')
    temporal.setAttribute('type', 'Constant')
    addParameter(temporal, 'Normalization', 0, value, 1.0, 0.1, 10.0)
    return temporal

def AddLCTrans(doc,filefun,value):
    temporal = doc.createElement('temporal')
    temporal.setAttribute('type', 'LightCurve')
    temporal.setAttribute('file', filefun)
    addParameter(temporal, 'Normalization', 0, value, 1.0, 0.0, 1000.0)
    return temporal
